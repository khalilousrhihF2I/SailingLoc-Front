...
import { boatService, destinationService, authService, adminService, homeService } from '../services/ServiceFactory';
...
export function EditBoatListingPage({ onNavigate, boatId, pageData }: EditBoatListingPageProps) {
  ...
  useEffect(() => {
    ...
    const loadBoat = async () => {
      ...
      try {
        ...
        const boatData: any = b;

        // Pré-remplir les formulaires
        setBasicInfo({
          name: boatData.name || '',
          type: boatData.type || 'sailboat',
          year: boatData.year || new Date().getFullYear(),
          length: boatData.length != null ? String(boatData.length) : '',
          capacity: boatData.capacity != null ? String(boatData.capacity) : '',
          cabins: boatData.cabins != null ? String(boatData.cabins) : '',
        });
        ...
        setLocation({
          ...
          destinationId: (boatData as any).destinationId ?? undefined,
        });

        setDetails({
          price: boatData.price != null ? String(boatData.price) : '',
        ...
        try {
          const imgs = (boatData as any).images ?? (boatData as any).Images ?? undefined;
          if (imgs) {
            if (typeof imgs === 'string') {
              try {
                const parsed = JSON.parse(imgs);
                if (Array.isArray(parsed)) setPhotos(parsed.filter(Boolean));
                else if (typeof parsed === 'string') setPhotos([parsed]);
              } catch {
        ...
        // equipment may be stored as JSON string in some APIs
        try {
          const eq = boatData.equipment && typeof boatData.equipment === 'string' ? JSON.parse(boatData.equipment) : boatData.equipment;
          setEquipment(eq || []);
        } catch {
          setEquipment(Array.isArray(boatData.equipment) ? boatData.equipment : []);
        }
      } catch (err) {
  ...
  // If pageData requests starting step (eg. admin linking directly to recap)
  useEffect(() => {
    try {
      if (pageData && pageData.startStep && Number(pageData.startStep) === 6) {
        setStep(6);
      }
    } catch { }
  }, [pageData]);

  // Detect admin role for extra actions
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        const role = u ? ((u as any).role || (u as any).type || ((Array.isArray((u as any).roles) && (u as any).roles[0]) ) || '') : '';
  ...
  // When boat or destinations load, pre-fill destination options and selection
  useEffect(() => {
    if (!allDestinations || allDestinations.length === 0) return;
    const currentCountry = boat?.country || location.country;
    if (currentCountry) {
      const filtered = allDestinations.filter((d: any) => d.country === currentCountry);
      setDestinationOptions(filtered || []);
      // If boat has destinationId, ensure it's selected
      const destId = (boat as any)?.destinationId ?? location.destinationId;
      if (destId) {
        setLocation((prev) => ({ ...prev, destinationId: destId }));
      }
    }
  }, [allDestinations, boat]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Ensure ownerId is a GUID string: prefer existing boat.ownerId, otherwise try current user
      ...
      if (boat && boat.ownerId && typeof boat.ownerId === 'string' && isGuid(boat.ownerId)) {
        ownerGuid = boat.ownerId;
      } else {
        try {
        ...
        <Card className="p-6 md:p-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {equipmentPresets.map((item) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg border-2 text-sm transition-all ${equipment.includes(item)
          ...
          {step === 5 && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-gray-600">Ajoutez des photos supplémentaires pour mettre en valeur votre bateau (optionnel)</p>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button type="button" onClick={() => { if (newPhotoUrl.trim() && photos.length < 8) { setPhotos([...photos, newPhotoUrl.trim()]); setNewPhotoUrl(''); } }} disabled={photos.length >= 8}>
                    Ajouter
                  </Button>
                </div>
                <p className="text-sm text-gray-500 mt-1">{photos.length} / 8 photos ajoutées</p>
              </div>
                    ...
                    {photos.map((url, index) => (
                      <div key={index} className="relative group">
                        <div className="w-full h-32 rounded-lg overflow-hidden">
                          <img src={url} alt={`Photo ${index + 1}`} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1567899378494-47b22a2ae96a?w=400'; }} />
                        </div>
                        <button type="button" onClick={() => setPhotos(photos.filter((_, i) => i !== index))} className="absolute top-2 right-2 w-8 h-8 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center hover:bg-red-600">×</button>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {step === 6 && (
            <div className="space-y-6">
              ...
              {/* Admin validate button in final step */}
              {isAdmin && step === 6 && (
                <div className="mt-4">
                  <Button variant="primary" onClick={() => {
                    modal.showConfirm({
                      title: 'Confirmer la validation',
                      message: "Voulez-vous vraiment valider cette annonce ? Cette action est définitive.",
                      confirmLabel: 'Confirmer et valider',
                      cancelLabel: 'Annuler',
                      onConfirm: async () => {
                        try {
                          if (!boat) throw new Error('Bateau non chargé');
                          setSaving(true);
                          if ((adminService as any) && (adminService as any).verifyBoat) {
                            await (adminService as any).verifyBoat(boat.id);
                          } else if ((boatService as any).setVerified) {
                            await (boatService as any).setVerified(boat.id, true);
                          } else {
                            throw new Error('Aucune API disponible pour valider l\'annonce');
                          }
                          setSuccess(true);
                          setTimeout(() => onNavigate('admin-dashboard'), 1200);
                        } catch (err: any) {
                  ...
                  }}>{saving ? 'Validation...' : 'Valider l\'annonce (admin)'}</Button>
                </div>
              )}
